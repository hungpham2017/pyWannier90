...
200 format ('UNK',i5.5,'.',i1)
    ...
    allocate(wann_func(0:ngs(1)*(ngx-1),0:ngs(2)*(ngy-1),0:ngs(3)*(ngz-1),num_wannier_plot),stat=ierr )
	!HP: Modified to for pyWannier90
             ...
             do nzz= 0, ngs(3)*(ngz-1)
                ...
                do loop_b=1,num_wann
                   do loop_w=1,num_wannier_plot            
                      wann_func(nzz,nyy,nxx,loop_w)=wann_func(nzz,nyy,nxx,loop_w)+ &
...
201   format (a,'_',i5.5,'.xsf')
      ...
      y_0ang= 0.0_dp
      z_0ang= 0.0_dp
      fxcry(1)=real(ngs(1),dp)
      fxcry(2)=real(ngs(2),dp)
      fxcry(3)=real(ngs(3),dp)
	...
	!Original	
      ...
      do loop_b=1,num_wannier_plot
         ...
         write(file_unit,'(3i6)') ngs(1)*(ngx-1)+1, ngs(2)*(ngy-1)+1, ngs(3)*(ngz-1)+1 !Original: ngs(1)*ngx, ngs(2)*ngy, ngs(3)*ngz || HP: ngs(1)*(ngx-1)+1, ngs(2)*(ngy-1)+1, ngs(3)*(ngz-1)+1 
